...
export class DashboardComponent implements OnInit {
  ...
  sendNonbillStackName(stackName) {
    console.log(stackName);
    this.nonbillservice.getTechnology(stackName);
  }
  count: any;
  expi: any;
  expidetails: any;
  ...
  revenuMap: any;
  profit: any;
  constructor(private billservice: BillService, private clientService: ClientService, private nonbillservice : NonbillService) {
    this.getBillDetails();
    this.getExpChart();
    ...
    console.log('profit deatils got successfully');
  });
...
getTechnologyWiseNonBillableEmployeesDetails() {
  let series = [];
  this.nonbillservice.getNonbillable().subscribe(res => {
//  console.log(res, ' res');
    this.details = res.list;

    this.nonbillservice.getTechnologyNonBillableEmployeesDetails().subscribe(countDeatils => {
...
getNonBillableEmpExpDetails() {
  let series = [];
  this.nonbillservice.getNonBillableEmployeesExpList().subscribe(experinaceList => {
    this.expi = experinaceList.expList;
    this.nonbillservice.getNonBillableEmployeesExpDetails().subscribe(expiCount => {
      ...
      const cart3 = Highcharts.chart('container4', {
        chart: {
          plotBackgroundColor: null,
          plotBorderWidth: 0,
          plotShadow: false
...
// console.log('revenu list::'+revenu);
